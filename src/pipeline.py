...
from src.load.load import save_to_csv, save_to_postgresql
...
def run_pipeline():
    ...
    if not pichau_cpu_t.empty:
        save_to_postgresql(pichau_cpu_t, 'cpus')
    
    if not pichau_gpu_t.empty:
        save_to_postgresql(pichau_gpu_t, 'gpus')
    
    if not kabum_cpu_t.empty:
        save_to_postgresql(kabum_cpu_t, 'cpus')
    
    if not kabum_gpu_t.empty:
        save_to_postgresql(kabum_gpu_t, 'gpus')
    
    if not terabyte_cpu_t.empty:
        save_to_postgresql(terabyte_cpu_t, 'cpus')
    
    if not terabyte_gpu_t.empty:
        save_to_postgresql(terabyte_gpu_t, 'gpus')
